...
def process_task(video_url, campaign_key, position_key, target_key, reply_webhook_url):
    ...
    try:
        ...
        # Check for cookies file to bypass Instagram blocks
        cookie_path = "/app/assets/cookies.txt"
        if os.path.exists(cookie_path):
            ydl_opts["cookiefile"] = cookie_path
            print("🍪 Found cookies.txt! Using it to bypass login blocks.", flush=True)
        else:
            print("⚠️ No cookies.txt found in /app/assets/. Instagram downloads may fail.", flush=True)
